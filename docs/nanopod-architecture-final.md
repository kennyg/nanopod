...
Before any operation, the system copies all files that will be modified to `.nanopod/backup/`. On success, the backup is deleted. On failure, the backup is restored. This provides rollback safety regardless of whether the user commits, pushes, or understands git.

---

## 1. The Shared Base

`.nanopod/base/` holds the clean core — the original codebase before any skills or customizations were applied. This is the stable common ancestor for all three-way merges, and it only changes on core updates.
...
- Auditable: `diff .nanopod/base/src/server.ts skills/add-whatsapp/modify/src/server.ts` shows exactly what the skill changes
- Deterministic: same three inputs always produce the same merge result
- Size is negligible since NanoPod's core files are small
...
applied_skills:
  - skill: telegram
    version: 1.0.0
    custom_patch: .nanopod/custom/telegram-group-only.patch
...
Copy all files that will be modified to `.nanopod/backup/`. If the operation fails at any point, restore from backup.
...
```bash
git merge-file src/server.ts .nanopod/base/src/server.ts skills/add-whatsapp/modify/src/server.ts
```
...
1. **Check shared resolution cache** (`.nanopod/resolutions/`) — load into local `git rerere` if a verified resolution exists for this skill combination. **Only apply if input hashes match exactly** (base hash + current hash + skill modified hash).
...
2. Update `.nanopod/state.yaml` — skill record, file hashes (base, skill, merged per file), structured outcomes
...
If tests pass, delete `.nanopod/backup/`. The operation is complete.

If tests fail and Level 2 can't resolve, restore from `.nanopod/backup/` and report the failure.
...
`git rerere` is local by default. But NanoPod has thousands of users applying the same skill combinations. Every user hitting the same conflict and waiting for Claude Code to resolve it is wasteful.

### The Solution

NanoPod maintains a verified resolution cache in `.nanopod/resolutions/` that ships with the project. This is the shared artifact — **not** `.git/rr-cache/`, which stays local.
...
git merge-file current.ts .nanopod/base/src/file.ts skills/add-whatsapp/modify/src/file.ts

# 2. If exit code > 0 (conflict), set up rerere adapter:

# Create blob objects for the three versions
base_hash=$(git hash-object -w .nanopod/base/src/file.ts)
...
`.nanopod/state.yaml` records everything about the installation:
...
custom_modifications:
  - description: "Added custom logging middleware"
    ...
    files_modified:
      - src/server.ts
    patch_file: .nanopod/custom/001-logging-middleware.patch
```
...
Core updates must be as programmatic as possible. The NanoPod team is responsible for ensuring updates apply cleanly to common skill combinations.
...
Copy all files that will be modified to `.nanopod/backup/`.
...
```bash
git merge-file src/server.ts .nanopod/base/src/server.ts updates/0.5.0-to-0.6.0/files/src/server.ts
```
...
```bash
git apply --3way .nanopod/custom/001-logging-middleware.patch
```
...
`.nanopod/base/` replaced with new clean core. This is the **only time** the base changes.
...
3. Backup the current codebase to `.nanopod/backup/`
...
2. Updates `.nanopod/base/` to the current core version's clean files
...
nanopod-init --version 0.5.0

# 2. Load shared resolutions into local rerere cache
load-resolutions .nanopod/resolutions/
...
for skill in state.applied_skills:
  ...
  for file in skill.files_modified:
    ...
    git merge-file ${resolved_path} .nanopod/base/${resolved_path} skills/${skill.name}/modify/${file}
...
Ship with NanoPod to reduce noisy merge conflicts:
...
5. **One shared base.** `.nanopod/base/` is the clean core before any skills or customizations. It's the stable common ancestor for all three-way merges. Only updated on core updates.
...
7. **Resolutions are learned and shared.** Maintainers resolve conflicts and ship verified resolutions with hash enforcement. `.nanopod/resolutions/` is the shared artifact.
