...
│  │    • mcp__nanopod__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── nanopod.log               # Host stdout
│   └── nanopod.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nanopod.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nanopod-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nanopod installation
...
NanoPod uses a hierarchical memory system based on CLAUDE.md files.
...
NanoPod has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__nanopod__schedule_task]
        {
...
### NanoPod MCP (built-in)

The `nanopod` MCP server is created dynamically per agent call with the current group's context.
...
NanoPod runs as a single macOS launchd service.

### Startup Sequence

When NanoPod starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NanoPod containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/nanopod.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanopod.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nanopod.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanopod` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NanoPod auto-starts container runtime but may fail |
...
- `logs/nanopod.log` - stdout
- `logs/nanopod.error.log` - stderr
