---
name: x-integration
description: X (Twitter) integration for NanoPod. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NanoPod v1.0.0. Directory structure may change in future versions.
...
1. **NanoPod is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep nanopod shows PID and exit code 0
```
...
| `NANOPOD_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nanopod.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NanoPod, make the following modifications:
...
All paths below are relative to project root (`NANOPOD_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep nanopod  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nanopod.log | tail -20

# Script errors
grep -i "error\|failed" logs/nanopod.log | tail -20
```
...
docker run nanopod-agent ls -la /app/src/skills/
```
