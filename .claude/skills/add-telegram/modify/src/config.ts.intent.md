...
## Key sections
- **readEnvFile call**: Must include `TELEGRAM_BOT_TOKEN` and `TELEGRAM_ONLY` in the keys array. NanoPod does NOT load `.env` into `process.env` — all `.env` values must be explicitly requested via `readEnvFile()`.
