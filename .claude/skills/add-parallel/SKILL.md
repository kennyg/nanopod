...
Adds Parallel AI MCP integration to NanoPod for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NanoPod scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NanoPod already set up and running
...
Add Parallel AI MCP servers after the nanopod server:
```typescript
const mcpServers: Record<string, any> = {
  nanopod: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__nanopod__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__nanopod__send_message
   4. Use mcp__nanopod__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nanopod-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/nanopod.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.nanopod`
