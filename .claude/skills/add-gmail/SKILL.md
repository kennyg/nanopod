---
name: add-gmail
description: Add Gmail integration to NanoPod. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to NanoPod. It can be configured in two modes:
...
> How do you want to use Gmail with NanoPod?
>
> **Option 1: Tool Mode**
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "NanoPod"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "NanoPod Gmail")
>    - Click **Create**
...
```bash
sleep 2 && launchctl list | grep nanopod
```
...
> - Create a Gmail label (e.g., "NanoPod")
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'NanoPod',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/nanopod.log | grep -i email
```
...
```bash
tail -f logs/nanopod.log | grep -E "(email|Email)"
```
