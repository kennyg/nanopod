...
case "$PLATFORM" in
  ...
  macos)
    PLIST_PATH="$HOME_PATH/Library/LaunchAgents/com.nanopod.plist"
...
<dict>
    ...
    <string>${PROJECT_PATH}/logs/nanopod.log</string>
    <key>StandardErrorPath</key>
    <string>${PROJECT_PATH}/logs/nanopod.error.log</string>
</dict>
</plist>
PLISTEOF
    ...
    if launchctl list 2>/dev/null | grep -q "com.nanopod"; then
      SERVICE_LOADED="true"
...
EOF
  ...
  linux)
    ...
    UNIT_PATH="$UNIT_DIR/nanopod.service"
    mkdir -p "$UNIT_DIR"
    log "Generating systemd unit at $UNIT_PATH"

    cat > "$UNIT_PATH" <<UNITEOF
[Unit]
Description=NanoPod Personal Assistant
...
StandardOutput=append:${PROJECT_PATH}/logs/nanopod.log
StandardError=append:${PROJECT_PATH}/logs/nanopod.error.log
...
UNITEOF
    ...
    systemctl --user enable nanopod >> "$LOG_FILE" 2>&1 || true
    systemctl --user start nanopod >> "$LOG_FILE" 2>&1 || true

    # Verify
    SERVICE_LOADED="false"
    if systemctl --user is-active nanopod >/dev/null 2>&1; then
      SERVICE_LOADED="true"
