...
if [ "$PLATFORM" = "macos" ]; then
  if launchctl list 2>/dev/null | grep -q "com.nanopod"; then
    ...
    LAUNCHCTL_LINE=$(launchctl list 2>/dev/null | grep "com.nanopod" || true)
  ...
  if systemctl --user is-active nanopod >/dev/null 2>&1; then
    SERVICE="running"
  elif systemctl --user list-unit-files 2>/dev/null | grep -q "nanopod"; then
    SERVICE="stopped"
  fi
fi
log "Service: $SERVICE"
...
if [ -f "$HOME/.config/nanopod/mount-allowlist.json" ]; then
