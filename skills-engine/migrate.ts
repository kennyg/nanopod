...
import { BASE_DIR, CUSTOM_DIR, NANOPOD_DIR } from './constants.js';
import { initNanopodDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initNanopodDir();
  console.log('Skills system initialized. .nanopod/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .nanopod/custom/migration.patch',
      );
    } else {
