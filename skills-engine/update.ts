...
import { BASE_DIR, NANOPOD_DIR } from './constants.js';
  ...
  try {
    ...
    for (const relPath of preview.filesChanged) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `nanopod-update-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
    ...
    const remapFile = path.join(newCorePath, '.nanopod-meta', 'path_remap.yaml');
