...
import { cleanup, createTempDir, initGitRepo, setupNanopodDir } from './test-helpers.js';
...
describe('update', () => {
  ...
  function writeStateFile(state: Record<string, unknown>): void {
    const statePath = path.join(tmpDir, '.nanopod', 'state.yaml');
        ...
        custom_modifications: [
          {
            ...
            patch_file: '.nanopod/custom/001-tweak.patch',
          },
        ],
      });
  ...
  describe('applyUpdate', () => {
    it('rejects when customize session is active', async () => {
      ...
      const customDir = path.join(tmpDir, '.nanopod', 'custom');
