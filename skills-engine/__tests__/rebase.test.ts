...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.nanopod', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.nanopod', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.nanopod', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase flattens: base updated to match working tree', async () => {
    ...
    const baseContent = fs.readFileSync(
      path.join(tmpDir, '.nanopod', 'base', 'src', 'index.ts'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
    // Set up base files
    const baseDir = path.join(tmpDir, '.nanopod', 'base');
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.nanopod/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.nanopod', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.nanopod', 'base', 'src', 'config.ts'),
      'utf-8',
    );
  ...
  it('rebase clears resolution cache', async () => {
    ...
    const resDir = path.join(tmpDir, '.nanopod', 'resolutions', 'skill-a+skill-b');
    ...
    const resolutions = fs.readdirSync(
      path.join(tmpDir, '.nanopod', 'resolutions'),
    );
