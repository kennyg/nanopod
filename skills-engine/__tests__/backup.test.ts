...
import { createTempDir, setupNanopodDir, cleanup } from './test-helpers.js';
...
describe('backup', () => {
  ...
  it('clearBackup removes backup directory', () => {
    ...
    const backupDir = path.join(tmpDir, '.nanopod', 'backup');
  ...
  it('createBackup writes tombstone for non-existent files', () => {
    ...
    const tombstone = path.join(tmpDir, '.nanopod', 'backup', 'src', 'newfile.ts.tombstone');
