...
import { createTempDir, setupNanopodDir, initGitRepo, cleanup } from './test-helpers.js';
...
describe('resolution-cache', () => {
  ...
  it('saveResolution writes file_hashes to meta.yaml', () => {
    ...
    const resDir = path.join(tmpDir, '.nanopod', 'resolutions', 'alpha+beta');
  ...
  describe('loadResolutions hash verification', () => {
    ...
    function setupInputFiles() {
      // Create base file
      fs.mkdirSync(path.join(tmpDir, '.nanopod', 'base', 'src'), { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.nanopod', 'base', 'src', 'config.ts'), baseContent);
