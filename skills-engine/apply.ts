...
import { NANOPOD_DIR } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, NANOPOD_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `nanopod-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
