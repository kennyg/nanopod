...
async function runMatrixEntry(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nanopod-ci-'));
  ...
  try {
    ...
    // Initialize nanopod dir
    execSync('npx tsx -e "import { initNanopodDir } from \'./skills-engine/index.js\'; initNanopodDir();"', {
      cwd: tmpDir,
      stdio: 'pipe',
      timeout: 30_000,
    });
